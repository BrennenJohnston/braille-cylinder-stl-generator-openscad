...
//  cylinder gets a matching keyed hole — so a gear cannot be seated in the wrong
//  place. This file cuts those holes.
//
//  IT FITS ONLY GEARS WITH R14 PEGS. The earlier star, hexagon and 15 x 15 mm
//  square pegs do not enter an R14 hole, so a cylinder printed from this file
//  pairs only with gears cut to the R14 spec.
//
...
// Slicer seam channel (OpenSCAD parity plan phase O2, 2026-09-21; web decisions
// D-1, D-2, D-13..D-15). Size is not a dial: the six SEAM_CHANNEL_* constants
// in the Hidden section mirror the web generator's app/geometry_spec.py and
...
// Version 2 has no polygonal cutout and no seam offset. They are declared here,
// out of the Customizer, because the shared geometry below still reads them.
// The keyed cutout is this barrel's only hole.
...
// =============================================================================
// INLINED PRESETS - BEGIN
// =============================================================================
// The Version 1 file reaches these tables with `include <presets.scad>;`. This
...
// returns [idx] on a hit but [[]] on a miss, so hit-vs-miss is discriminated
// with is_num(m[0]).
function preset_lookup(preset_list, key) =
    let (m = search([key], preset_list))
    is_num(m[0]) ? preset_list[m[0]][1] : undef;

// Public helper: the table value when the preset matches, otherwise the
// caller's fallback (typically a slider variable).
...
// DS_GAP_RELIABLE (0.45 mm, provisional) and DS_GAP_FLOOR (0.34 mm).
...
// Tactile arrow lead-in (2026-09-21, web decisions D-T1..D-T4 after a printed
// card): the embosser is loaded with the card's leading edge AT the alignment
// arrow, so the arrow no longer sits at the seam-gap midpoint but a fixed
// lead-in before the first cell - half its width, the recess clearance, this
// margin and the cell's dot footprint - and whatever the gap has to spare
// falls after the last cell, where it costs nothing. Mirrors the web
// generator's TACTILE_LEAD_IN_MARGIN_MM (tests/test_tactile_mode.py diffs it):
// the same 1 mm the clear-zone rule carries. Never lower it - it is the
...
// its leading edge sits: the 90 mm business card the web generator carries as
// card_width. A constant, not a dial - the web app reads its own field.
CARD_LENGTH_MM = 90;
...
// Normalised gate for the `seam_channel` dropdown, read the way ds_on reads
...
// 2026-09-20). Both presets set the cylinder to 54 mm here, so the shipped
// defaults pass at every slider setting (the slider maxima need 45 mm).
...
// this gap is always exactly 180°; since 2026-09-21 the tactile indicator sits a
// fixed lead-in before the first cell instead (tactile_arrow_theta_emboss_deg,
// below the seam-channel footprint). Warn when the gap can no longer hold the
// indicator plus a clear zone either side of it.
...
// ported verbatim, the same block the Version 1 file carries. Positions are
// signed arcs `s` along the surface from the seam centre (180 degrees),
// positive TOWARD column 0; the free window [lo, hi] is what the features
// either side leave of the seam gap and the groove sits at its middle so both
// margins are equal. The footprint is the SAME number on both plates, so the
// two grooves mirror exactly.
//
...
// carries. Never treat this angle differently from a dot's.
...
// Tactile arrow lead-in (TACTILE_LEAD_IN_MARGIN_MM): the arrow centre sits this
// far before the first cell centre, and its arc from the seam centre toward
// column 0 is whatever the half gap has to spare - never negative: a gap too
// small to honour the lead-in on both sides keeps the arrow at 180 and the
// tactile gap warning above speaks. PHYSICAL angles, exactly the seam channel's
// rule below: column 0 sits on the negative side of 180 on the emboss plate, so
// toward it is INCREASING angle there and decreasing on the counter plate -
// whose recess is placed OUTSIDE the mirror at its own angle, so the two still
// meet at the nip. The web generator's tactile_arrow_theta() is the same
// number in its dot convention (pi -/+ s/R), negated by its worker.
tactile_lead_in_mm = tactile_indicator_width / 2 + tactile_recess_clearance
    + TACTILE_LEAD_IN_MARGIN_MM + seam_channel_footprint_mm;
tactile_arrow_arc_mm = max(0, seam_gap_mm / 2 - tactile_lead_in_mm);
...
if (tactile_on) {
    echo(str("NOTE: tactile arrow lead-in ", tactile_lead_in_mm, " mm; arrow at ",
             tactile_arrow_theta_emboss_deg, " deg on the emboss plate, ",
             tactile_arrow_theta_counter_deg, " deg on the counter plate."));
}

// Card fit: with the card's leading edge at the arrow, a tactile row needs the
// lead-in plus the grid plus the last cell's footprint of card. 13 cells at the
// 0.4mm preset need 85.5 mm and fit a 90 mm card; 14 need 92.0 and lose their
// last cell - which is how this rule was found. A NOTE and a red badge, never a
// stop: the cylinder itself still holds the row.
...
tactile_card_max_cells = max(0,
    floor((CARD_LENGTH_MM - tactile_lead_in_mm - seam_channel_footprint_mm) / active_cell_spacing) + 1);
...
// The groove's window. Tactile, since the lead-in: BEHIND the arrow, between
// the last cell's dots - the back grid's, an interpoint offset closer to the
// seam, when double-sided - and the (clearance-grown) arrow recess; the lead-in
// side keeps only its 1 mm margin. Visual: unchanged.
seam_channel_lo_mm = tactile_on
    ? -(seam_gap_mm / 2 - seam_channel_footprint_mm - (ds_on ? interpoint_offset_x_mm : 0))
    : -(seam_gap_mm / 2 - seam_channel_footprint_mm);                  // past the last cell's dots
seam_channel_hi_mm = tactile_on
    ? (tactile_arrow_arc_mm - (tactile_indicator_width / 2 + tactile_recess_clearance))
...
// The wall under the apex. A Version 2 barrel is solid - the keyed hole is
// its only bore and no key corner reaches past 10.7 mm from the axis, the
// nub 14.3 and the socket 14.0, all inside the 14.9 mm groove floor - and
// the polygon cutout is fixed at 0 in the Hidden section, so the rule is kept
// for parity with the Version 1 file and can only ever pass here.
...
// =============================================================================
// Every number below mirrors app/geometry/version2.py in the web repository,
// name for name; Phase 12's tests compare the two. Family R14 is four rounded
// rectangles whose LONG dimension lies on 90/270 degrees, so a flat always
// faces the tactile arrow column.

V2_KEY_CORNER_R = 0.5;          // corner radius of every key, mm
V2_KEY_A1 = [14, 14];           // Cylinder A TOP  (the nub end) [length, width]
V2_KEY_A2 = [18, 10];           // Cylinder A BOTTOM
V2_KEY_B1 = [16, 12];           // Cylinder B TOP
V2_KEY_B2 = [20, 8];            // Cylinder B BOTTOM
V2_COUNTERSINK_OFFSET = 2.0;    // mouth flare at the face, mm
V2_COUNTERSINK_DEPTH = 2.0;     // and how deep it reaches, mm - equal, so 45 degrees
V2_OVERLAP = 0.01;              // overlap, never touch, mm
V2_ARC_FN = 96;                 // segments per full circle on a key corner
V2_ARROW_COLUMN = 180;          // the program angle the row indicators use
V2_SLAB = 0.01;                 // thickness of a hull slab, mm

// The key nub on Cylinder A's top face, reproduced from Brennen's CAD as
// measured - including the fact that it is not exactly equilateral.
V2_NUB_SIDE = 5.073158;
V2_NUB_BASE_R = 9.754087;
...
// out at exactly 0.5 + c. Built as offset(r = ...) around a square inset by
// that radius - NOT offset(delta = c) on an already-rounded profile, which
// pushes each vertex of a TESSELLATED arc out by c/cos(180/V2_ARC_FN) rather
// than c (0.6500804 instead of 0.6500000 at the default clearance).
module key_profile_2d(key, clearance) {
    r = V2_KEY_CORNER_R + clearance;
    offset(r = r, $fn = V2_ARC_FN)
        square([key[1] - 2 * V2_KEY_CORNER_R, key[0] - 2 * V2_KEY_CORNER_R], center = true);
}

// The nub outline, inset by the clearance. offset(delta = ...) IS exact here,
// because this triangle's edges are genuinely straight. A miter inset moves
// every FACE in by c, which on this triangle costs the base half-width
// sqrt(3)*c - the inradius is what drops by exactly c.
module nub_2d(clearance) {
...
// so no two solids ever share an exact plane.
...
// outline V2_COUNTERSINK_DEPTH in. ONE rule covers all four mouths.
//
// SLAB PLACEMENT IS LOAD-BEARING. A hull's tapering face is supported by the
// FAR edge of each slab, so each slab sits with its far edge exactly at the end
// of its own taper. Placed the other way round the 45 degree taper overshoots
// by the slab's own thickness.
module mouth_countersink(key, face_z, into_plus_z) {
    dir = into_plus_z ? 1 : -1;
    // Face slab: its far edge is AT the face. Inner slab: its far edge is at
    // the full depth.
    face_lo  = into_plus_z ? face_z : face_z - V2_SLAB;
    inner_lo = into_plus_z ? face_z + V2_COUNTERSINK_DEPTH - V2_SLAB
                           : face_z - V2_COUNTERSINK_DEPTH;
    hull() {
...
// The key nub: a flared base, a straight body and a chamfered top, built as
// THREE parts UNIONED and never one hull. The nub widens at its base and
// narrows again at its top, so it is not convex: a single hull bridges straight
// from the flare to the chamfer and bulges the body out by 0.2 mm - measured
...
module top_key_nub(top_face_z, emboss) {
    ...
    body_hi = z1 - V2_NUB_TOP_CHAMFER;

    union() {
        // Flared base: far edges out, as with the mouths.
        hull() {
...
// sides through the surface at SEAM_CHANNEL_WIDTH_MM and on out to a lip
// beyond it so the mouth is cut rather than touched - extruded the full
// height plus an overshoot past each end face, then turned to the groove's
// physical angle. Subtracted from the BARE outer cylinder before the keyed
// halves, the mouths, the socket and anything unioned on, so a raised arrow
...
// below. For a rounded rectangle the corner centre sits at
// (length/2 - r, width/2 - r) and the outline reaches r + c beyond it.
function v2_key_max_radius(key, clearance) =
...
// Version 2 has no polygonal cutout, so the wall this guard protects is the one
// between the tactile arrow's recess and the KEYED HOLE. The widest key corner
// is what matters - B's 20 x 8 bottom key reaches furthest from the axis.
...
// Radial prism straddling the shell surface at the plate's arrow angle (the
// seam-gap centre until 2026-09-21; a lead-in before column 0 since - see
// tactile_arrow_theta_emboss_deg / _counter_deg). Passing cyl_radius = radius +
...
module tactile_surface_prism(y_pos, span, theta_deg) {
    place_cylinder_marker(theta_deg, y_pos, radius + span / 2, span, 0)
...
// The Version 1 polygonal shell is deliberately ABSENT from this file. Version 2's
// barrel is cylinder_shell_v2() above: solid, with the keyed hole as its only bore.
...
module cylinder_counter_plate() {
    translate([0, 0, active_cylinder_height_mm/2]) {
        difference() {
            ...
            // into. Placed OUTSIDE the mirror at this plate's own angle
            // (tactile_arrow_theta_counter_deg, the emboss angle reflected about
            // 180), so it lands on the emboss arrow when the pair is in its
            // paired pose - as it did when both sat at 180, the mirror's own
            // fixed point.
            if (tactile_on) {
