...
//  and double-sided cards — matches the Version 1 file, with ONE trim: the
...
//  • Card Sides (the double_sided switch, Back_Line_1 - Back_Line_4, and the
//    two interpoint offsets)
//  • Gears (the integrated_gears switch)
...
//  • Version 2 Keyed Cutouts (the key clearance)
//
//  OPENSCAD-SPECIFIC:
//  • Rendering Quality
//
...
// cone. Cut from the BARE barrel in cylinder_shell_v2 like the straight
// groove: the path keeps SEAM_CHANNEL_MARGIN_MM clear of every arrow, so the
// arrows unioned on afterwards can neither fill it back nor be cut by it.
...
module cylinder_shell_v2(emboss, channel_theta_deg = undef, channel_path = undef) {
